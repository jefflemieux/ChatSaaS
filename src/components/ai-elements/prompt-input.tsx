  ...
  return (
    ...
    >
      {data.mediaType?.startsWith("image/") && data.url ? (
        ...
        <div className="flex flex-col size-full items-center justify-center text-muted-foreground gap-2">
          <PaperclipIcon className="size-4" />
          <span className="text-xs">{data.filename?.slice(0, 3)}...{data.filename?.slice(-6)}</span>
        </div>
      )}
      <Button
        aria-label="Remove attachment"
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 rounded-full group-hover:opacity-100"
        onClick={() => attachments.remove(data.id)}
        size="icon"
        type="button"
        variant="outline"
      >
        <XIcon className="h-[3px] w-[3px]" />
      </Button>
    </div>
  );
}
